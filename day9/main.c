...
long long int processline(long long int *numbers, int size){
    ...
    if(zeros){
        ...
        //printf("%lld\n", linedown[0] - processline(linedown, size));
        return linedown[i-1] + processline(linedown, size);
    }
}

long long int processline2(long long int *numbers, int size){
    long long int *linedown = (long long int *)malloc(sizeof(long long int) * size);
    int i = 0;
    int zeros = true;
    while(numbers[i+1] != -696969){
    ...
    if(zeros){
        ...
        return linedown[0] - processline2(linedown, size);
    }
}
...
int main(int argc, char *argv[]){ 
    ...
    while((read = getline(&line, &len, fp)) != -1){
        ...
        long long int next = processline2(mynums, 200);
        ...
        long long int nextNum = mynums[0] - next;
        total += nextNum;
        printf("%d, %lld, %lld\n", lineNum++, mynums[0], next);
        //printf("%d");
    }
